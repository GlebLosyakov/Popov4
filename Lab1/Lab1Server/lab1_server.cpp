...
void handleClient(SOCKET clientSocket, sockaddr_in clientAddr) {
    ...
    while (true) {
        ...
        // Проверяем первый бит для определения типа сообщения
        bool isEncrypted = (buffer[0] & 0x01) != 0;

        std::string received_msg(buffer + 1, bytesReceived - 1); // Пропускаем первый байт с флагом
        std::string decrypted_msg;

        if (isEncrypted) {
            // Дешифрование данных
            crypt(received_msg, decrypted_msg, password);
            std::cout << "Received ENCRYPTED message (" << (bytesReceived - 1) << " bytes)" << std::endl;
        }
        else {
            // Сообщение не зашифровано
            decrypted_msg = received_msg;
            std::cout << "Received PLAIN message (" << (bytesReceived - 1) << " bytes)" << std::endl;
        }
        ...
        // Подготовка ответа (всегда отправляем зашифрованный ответ для совместимости)
        std::string response = "Server received: " + decrypted_msg;
        std::string encrypted_response;

        // Добавляем флаг шифрования (1 - зашифровано)
        char flag = 0x01;
        encrypted_response += flag;

        // Шифрование ответа
        std::string temp_encrypted;
        crypt(response, temp_encrypted, password);
        encrypted_response += temp_encrypted;
...
int main() {
    std::cout << "=== TCP Server with Optional Encryption ===" << std::endl;
